...
public class UserManage extends JInternalFrame {
    ...
    private UserManage() {
        ...
        panelButton.setBounds(10, 11, 244, 120);
        ...
        panelFind.setBounds(289, 11, 289, 120);
        ...
        ImageIcon iconReset = ImagesUtils.createResizedIcon("/images/resetButtonIcon.png", 45, 45);
        ...
        btnReset.setMargin(new Insets(10, 0, 10, 0));
        btnReset.setHorizontalTextPosition(SwingConstants.CENTER);
        btnReset.setFont(new Font("Tahoma", Font.BOLD, 14));
        btnReset.setFocusPainted(false);
        btnReset.setBorder(null);
        btnReset.setBackground(Color.WHITE);
        btnReset.setBounds(158, 22, 63, 88);
        ButtonUtils.setupButtonForJIFrame(btnReset);
	...
	protected void btnResetActionPerformed(ActionEvent e) {
		int selectedRow = table.getSelectedRow();
		if (selectedRow == -1) {
	        ShowMessage.showWarningMessage("Warning", "Please Choose a row for Reset Password");
	    } else {
	    	
	    	int option = JOptionPane.showConfirmDialog(this, "Are you sure you want to Reset Password for this user?", "Confirm Deletion", JOptionPane.YES_NO_OPTION);
	    	
	    	if (option == JOptionPane.YES_OPTION) {
	    		var userDAO = new UserDAO();
					boolean isSuccess = userDAO.resetPassword(
						table.getValueAt(selectedRow, 0).toString());
					refreshUsers();
					if(isSuccess) {
						ShowMessage.showMessage("Message", "Password reset to 00000 successfully");
					} else {
						ShowMessage.showErrorMessage("Error", "Cannot Reset Password, some error happened!");
					}
	        }
	    }
	}
